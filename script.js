document.addEventListener('DOMContentLoaded', () => {
  ...
  $('vrBackBtn').addEventListener('click', goHome);
  ...
  function renderGraph() {
    ...
    const h = ctx.canvas.parentElement.offsetHeight || 460;
    const areaGrad = ctx.createLinearGradient(0, 0, 0, h);
    areaGrad.addColorStop(0, 'rgba(124,58,237,0.18)');
    areaGrad.addColorStop(0.6, 'rgba(124,58,237,0.04)');
  ...
  function buildFilteredListCanvases(selected, opts = {}) {
    const width = 1240;
    const height = 1754;
    const canvases = [];
    ...
    const cols = [50, 170, 130, tableW - (50 + 170 + 130 + 170), 170];
    const rowH = 29;
    ...
    while (rowIndex < selected.length || (selected.length === 0 && page === 1)) {
      ...
      ctx.fillText(page === 1 ? 'Filtered Invoice List' : 'Filtered Invoice List (Continued)', margin, y);
      y += 50;
      ...
      headers.forEach((h, i) => {
        const tx = i === 4 ? x + cols[i] - 10 - ctx.measureText(h).width : x + 10;
      ...
      if (selected.length === 0) {
        ...
        ctx.fillText('No invoices found for the selected filters.', margin + 10, y + 30);
      }
  ...
  // ── Shared: safe HTML escaping for analytics PDF content ──
  function escHtml(str) {
    return String(str)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
  ...
  function computeProductSalesStats(from, to) {
    var src = (from && to)
  ...
  function renderProductSales(from, to) {
    from = (from !== undefined) ? from : psCurrentFrom;
    to   = (to   !== undefined) ? to   : psCurrentTo;
    psCurrentFrom = from;
    psCurrentTo   = to;
    ...
    $('psKpiProducts').textContent = visible.length;
    $('psKpiInvoices').textContent = totalInvoiceCount;
    $('psKpiQty').textContent      = fmtNum(visible.reduce(function(s, r) { return s + r.totalQty; }, 0));
    $('psKpiRevenue').textContent  = '₹' + fmtNum(visible.reduce(function(s, r) { return s + r.totalRevenue; }, 0));

    var tbody = $('psTableBody');
    if (!visible.length) {
      tbody.textContent = '';
      $('psEmpty').style.display = 'block';
      return;
    }
    $('psEmpty').style.display = 'none';

    var totalQtyAll     = visible.reduce(function(s, r) { return s + r.totalQty; }, 0);
    var totalRevenueAll = visible.reduce(function(s, r) { return s + r.totalRevenue; }, 0);
    var totalCountAll   = visible.reduce(function(s, r) { return s + r.invoiceCount; }, 0);
    ...
    visible.forEach(function(r, i) {
      ...
      var tdRate = document.createElement('td'); tdRate.className = 'r';
      ...
      var tdRev = document.createElement('td'); tdRev.className = 'r';
      tdRev.style.cssText = 'font-weight:600;color:#6d28d9';
      tdRev.textContent = '₹' + fmtNum(r.totalRevenue);
  ...
  function applyPsPreset(range, btnId) {
    if (psActivePreset === btnId) { clearPsPreset(); renderProductSales('', ''); return; }
    clearPsPreset();
    psActivePreset = btnId;
    $(btnId).classList.add('active');
    $('psPresetClear').style.display = 'inline-flex';
    renderProductSales(range.from, range.to);
  }

  $('psPresetToday').addEventListener('click',     function() { applyPsPreset(getDayRange(0),    'psPresetToday'); });
  $('psPresetYesterday').addEventListener('click', function() { applyPsPreset(getDayRange(-1),   'psPresetYesterday'); });
  $('psPresetThisWeek').addEventListener('click',  function() { applyPsPreset(getWeekRange(0),   'psPresetThisWeek'); });
  $('psPresetLastWeek').addEventListener('click',  function() { applyPsPreset(getWeekRange(-1),  'psPresetLastWeek'); });
  $('psPresetThisMonth').addEventListener('click', function() { applyPsPreset(getMonthRange(0),  'psPresetThisMonth'); });
  ...
  $('psDownloadPdfBtn').addEventListener('click', async function() {
    ...
    var presetNames = { psPresetToday:'Today', psPresetYesterday:'Yesterday', psPresetThisWeek:'This week',
      psPresetLastWeek:'Last week', psPresetThisMonth:'This month', psPresetLastMonth:'Last month',
      psPresetThisYear:'This year', psPresetLastYear:'Last year' };
    var periodLabel = (psActivePreset && presetNames[psActivePreset])
      ? presetNames[psActivePreset]
      : (psCurrentFrom && psCurrentTo ? formatShortDate(psCurrentFrom) + ' – ' + formatShortDate(psCurrentTo) : 'All dates');
    ...
    var titleEl  = document.createElement('div'); titleEl.style.cssText = 'font-size:18px;font-weight:700;color:#1e293b'; titleEl.textContent = 'Sales Analytics';
    ...
    visible.forEach(function(r, i) {
      ...
      [i + 1, r.name, r.invoiceCount, fmtNum(r.totalQty),
       '₹' + fmtNum(r.avgRate) + (r.rateVaried ? ' (varied)' : ''),
       '₹' + fmtNum(r.totalRevenue)].forEach(function(val, ci) {
        var cell = document.createElement('td');
        cell.style.cssText = ci >= 2 ? tdr : td;
        if (ci === 5) { cell.style.fontWeight = '600'; cell.style.color = '#6d28d9'; }
        cell.textContent = val;
        row.appendChild(cell);
      });
      tb.appendChild(row);
    });
    var totRow2 = document.createElement('tr'); totRow2.style.background = '#eef2ff';
    var tc1 = document.createElement('td'); tc1.colSpan = 2; tc1.style.cssText = td + 'font-weight:700;color:#4338ca'; tc1.textContent = 'Totals'; totRow2.appendChild(tc1);
    [totalCount, fmtNum(totalQty), '—', '₹' + fmtNum(totalRevenue)].forEach(function(val, ci) {
      var c = document.createElement('td'); c.style.cssText = tdr + 'font-weight:700;color:' + (ci === 2 ? '#94a3b8' : '#4338ca');
    ...
    var fname = 'sales-analytics-' + (psCurrentFrom || 'all') + '.pdf';
  ...
  // ── Purchase Analytics ────────────────────────────────────
  var paActivePreset = null;
  var paCurrentFrom  = '';
  var paCurrentTo    = '';

  function computePurchaseAnalyticsStats(from, to) {
    var src = (from && to)
  ...
  function renderPurchaseAnalytics(from, to) {
    from = (from !== undefined) ? from : paCurrentFrom;
    to   = (to   !== undefined) ? to   : paCurrentTo;
    paCurrentFrom = from;
    paCurrentTo   = to;
    ...
    $('paKpiProducts').textContent = visible.length;
    $('paKpiInvoices').textContent = totalInvoiceCount;
    $('paKpiQty').textContent      = fmtNum(visible.reduce(function(s, r) { return s + r.totalQty; }, 0));
    $('paKpiRevenue').textContent  = '₹' + fmtNum(visible.reduce(function(s, r) { return s + r.totalRevenue; }, 0));
    ...
    visible.forEach(function(r, i) {
      ...
      var tdRev = document.createElement('td'); tdRev.className = 'r';
      tdRev.style.cssText = 'font-weight:600;color:#0284c7';
      tdRev.textContent = '₹' + fmtNum(r.totalRevenue);
      tr.appendChild(tdRev);
    ...
    var totRow = document.createElement('tr'); totRow.className = 'analytics-totals-row';
    var pt1 = document.createElement('td'); pt1.colSpan = 2; pt1.textContent = 'Totals'; totRow.appendChild(pt1);
    var pt2 = document.createElement('td'); pt2.className = 'r'; pt2.textContent = totalCountAll; totRow.appendChild(pt2);
    var pt3 = document.createElement('td'); pt3.className = 'r'; pt3.textContent = fmtNum(totalQtyAll); totRow.appendChild(pt3);
    var pt4 = document.createElement('td'); pt4.className = 'r'; pt4.textContent = '—'; totRow.appendChild(pt4);
    var pt5 = document.createElement('td'); pt5.className = 'r'; pt5.textContent = '₹' + fmtNum(totalRevenueAll); totRow.appendChild(pt5);
    fragment.appendChild(totRow);
  ...
  function applyPaPreset(range, btnId) {
    if (paActivePreset === btnId) { clearPaPreset(); renderPurchaseAnalytics('', ''); return; }
    clearPaPreset();
    paActivePreset = btnId;
    $(btnId).classList.add('active');
    $('paPresetClear').style.display = 'inline-flex';
    renderPurchaseAnalytics(range.from, range.to);
  }

  $('paPresetToday').addEventListener('click',     function() { applyPaPreset(getDayRange(0),    'paPresetToday'); });
  $('paPresetYesterday').addEventListener('click', function() { applyPaPreset(getDayRange(-1),   'paPresetYesterday'); });
  $('paPresetThisWeek').addEventListener('click',  function() { applyPaPreset(getWeekRange(0),   'paPresetThisWeek'); });
  $('paPresetLastWeek').addEventListener('click',  function() { applyPaPreset(getWeekRange(-1),  'paPresetLastWeek'); });
  $('paPresetThisMonth').addEventListener('click', function() { applyPaPreset(getMonthRange(0),  'paPresetThisMonth'); });
  ...
  $('paDownloadPdfBtn').addEventListener('click', async function() {
    ...
    var presetNames = { paPresetToday:'Today', paPresetYesterday:'Yesterday', paPresetThisWeek:'This week',
      paPresetLastWeek:'Last week', paPresetThisMonth:'This month', paPresetLastMonth:'Last month',
      paPresetThisYear:'This year', paPresetLastYear:'Last year' };
    var periodLabel = (paActivePreset && presetNames[paActivePreset])
      ? presetNames[paActivePreset]
      : (paCurrentFrom && paCurrentTo ? formatShortDate(paCurrentFrom) + ' – ' + formatShortDate(paCurrentTo) : 'All dates');
    ...
    var titleDiv = document.createElement('div'); titleDiv.style.marginBottom = '16px';
    var titleEl  = document.createElement('div'); titleEl.style.cssText = 'font-size:18px;font-weight:700;color:#1e293b'; titleEl.textContent = 'Purchase Analytics';
    var periodEl = document.createElement('div'); periodEl.style.cssText = 'font-size:13px;color:#64748b;margin-top:2px'; periodEl.textContent = 'Period: ' + periodLabel;
    titleDiv.appendChild(titleEl); titleDiv.appendChild(periodEl); wrapper.appendChild(titleDiv);
    ...
    table.insertAdjacentHTML('afterbegin',
      ...
      + '<th style="' + thl + 'width:32px">#</th>'
      + '<th style="' + thl + '">Product Name</th>'
      + '<th style="' + thr + '">PO Count</th>'
      + '<th style="' + thr + '">Total Qty</th>'
      + '<th style="' + thr + '">Avg Rate</th>'
      + '<th style="' + thr + '">Total Spend</th>'
      + '</tr></thead>');
    var tb = document.createElement('tbody');
    visible.forEach(function(r, i) {
      var row = document.createElement('tr');
      [i + 1, r.name, r.invoiceCount, fmtNum(r.totalQty),
       ...
       '₹' + fmtNum(r.totalRevenue)].forEach(function(val, ci) {
        var cell = document.createElement('td');
        cell.style.cssText = ci >= 2 ? tdr : td;
        if (ci === 5) { cell.style.fontWeight = '600'; cell.style.color = '#0284c7'; }
        cell.textContent = val;
        row.appendChild(cell);
      });
      tb.appendChild(row);
    });
    var paTotal = document.createElement('tr'); paTotal.style.background = '#e0f2fe';
    var pc1 = document.createElement('td'); pc1.colSpan = 2; pc1.style.cssText = td + 'font-weight:700;color:#0369a1'; pc1.textContent = 'Totals'; paTotal.appendChild(pc1);
    [totalCount, fmtNum(totalQty), '—', '₹' + fmtNum(totalRevenue)].forEach(function(val, ci) {
      var c = document.createElement('td'); c.style.cssText = tdr + 'font-weight:700;color:' + (ci === 2 ? '#94a3b8' : '#0369a1');
      c.textContent = val; paTotal.appendChild(c);
    });
    tb.appendChild(paTotal);
    table.appendChild(tb); wrapper.appendChild(table);
  ...
  function crInvTaxable(inv) {
    var sub = 0;
    (inv.items || []).forEach(function(it) { sub += numericQtyForLine(it) * (Number(it.rate) || 0); });
    return sub;
  }
  ...
  function computeCustomerReportStats(from, to, cmpFrom, cmpTo) {
    ...
    src.forEach(function(inv) {
      ...
      var taxable = crInvTaxable(inv), gst = crInvGst(inv);
      map[key].taxable    += taxable;
      map[key].gstAmount  += gst;
      map[key].grandTotal += taxable + gst;
  ...
  function openCrDrawer(r) {
    ...
    r.invList.slice().sort(function(a, b) {
      return (b.invoiceDate || '') > (a.invoiceDate || '') ? 1 : -1;
    }).forEach(function(inv, i) {
      var taxable  = crInvTaxable(inv);
      ...
      var sp = document.createElement('span'); sp.className = 'cr-inv-no'; sp.textContent = inv.invoiceNumber || '—';
      c1.appendChild(sp); tr.appendChild(c1);

      var c2 = document.createElement('td'); c2.className = 'r';
      c2.textContent = inv.invoiceDate ? formatShortDate(inv.invoiceDate) : '—'; tr.appendChild(c2);

      var c3 = document.createElement('td'); c3.className = 'r';
      c3.textContent = '₹' + fmtNum(taxable); tr.appendChild(c3);

      var c4 = document.createElement('td'); c4.className = 'r';
      var tag = document.createElement('span'); tag.className = 'cr-gst-tag'; tag.textContent = gstLabel;
      var br  = document.createElement('br');
      var amt = document.createTextNode('₹' + fmtNum(gst));
      c4.appendChild(tag); c4.appendChild(br); c4.appendChild(amt); tr.appendChild(c4);

      var c5 = document.createElement('td'); c5.className = 'r cr-grand-cell';
      c5.textContent = '₹' + fmtNum(grand); tr.appendChild(c5);

      tb.appendChild(tr);
    });

    $('crDrawerBackdrop').classList.add('cr-active');
    $('crDrawer').classList.add('cr-active');
    document.body.style.overflow = 'hidden';
  }

  function closeCrDrawer() {
    $('crDrawer').classList.remove('cr-active');
    $('crDrawerBackdrop').classList.remove('cr-active');
  ...
  function renderCustomerReport(from, to) {
    ...
    visible.forEach(function(r, i) {
      ...
      tr.title = 'Click to see GST breakdown';

      var tdNum = document.createElement('td'); tdNum.textContent = i + 1; tr.appendChild(tdNum);

      var tdName = document.createElement('td'); tdName.className = 'cr-name-td';
      var nameSpan = document.createElement('span'); nameSpan.className = 'cr-name-cell'; nameSpan.textContent = r.name;
      ...
      path.setAttribute('d', 'M9 18l6-6-6-6'); arrow.appendChild(path);
      ...
      (function(row) { tr.addEventListener('click', function() { openCrDrawer(row); }); }(r));
      fragment.appendChild(tr);
    });
  ...
  function renderAgingReport() {
    ...
    rows.forEach(function(r, i) {
      ...
      var td0 = document.createElement('td'); td0.style.verticalAlign = 'top';
      var n0a = document.createElement('div'); n0a.textContent = i + 1;
      var n0b = document.createElement('div'); n0b.className = 'aging-inv-count'; n0b.textContent = ' ';
      td0.appendChild(n0a); td0.appendChild(n0b); tr.appendChild(td0);

      var td1 = document.createElement('td'); td1.style.verticalAlign = 'top';
      var nameRow = document.createElement('div'); nameRow.className = 'aging-name-row';
      ...
      nameRow.appendChild(sp); nameRow.appendChild(arrow);
      var nameSub = document.createElement('div'); nameSub.className = 'aging-inv-count';
      nameSub.textContent = totalInvCount + ' inv outstanding';
      td1.appendChild(nameRow); td1.appendChild(nameSub); tr.appendChild(td1);

      var td2 = document.createElement('td'); td2.className = 'r cr-grand-cell'; td2.style.verticalAlign = 'top';
      var outAmt = document.createElement('div'); outAmt.textContent = '\u20b9' + fmtNum(r.total);
      var outCnt = document.createElement('div'); outCnt.className = 'aging-inv-count'; outCnt.textContent = totalInvCount + ' inv';
      td2.appendChild(outAmt); td2.appendChild(outCnt); tr.appendChild(td2);

      r.b.forEach(function(v, bi) {
        var td = document.createElement('td'); td.className = 'r'; td.style.verticalAlign = 'top';
        var line1 = document.createElement('div');
        var line2 = document.createElement('div'); line2.className = 'aging-inv-count';
        if (v > 0.005) {
          td.classList.add('aging-cell-' + bi);
          line1.textContent = '\u20b9' + fmtNum(v); line2.textContent = r.bc[bi] + ' inv';
        } else {
          line1.textContent = '\u2014'; line1.style.color = '#c9c4d4'; line2.textContent = ' ';
        }
        td.appendChild(line1); td.appendChild(line2); tr.appendChild(td);
      });

      var td7 = document.createElement('td'); td7.className = 'r'; td7.style.verticalAlign = 'top';
      var dateLine = document.createElement('div'); dateLine.textContent = r.oldestDate ? formatShortDate(r.oldestDate) : '\u2014';
      var dateSub = document.createElement('div'); dateSub.className = 'aging-inv-count'; dateSub.textContent = ' ';
      td7.appendChild(dateLine); td7.appendChild(dateSub); tr.appendChild(td7);

      (function(row) { tr.addEventListener('click', function() { openAgingDrawer(row); }); }(r));
  ...
  function openAgingDrawer(row) {
    ...
    var buckets = [
      { idx: 3, label: '90+ days',    cls: '3', items: [] },
      { idx: 2, label: '61–90 days',  cls: '2', items: [] },
      { idx: 1, label: '31–60 days',  cls: '1', items: [] },
      { idx: 0, label: '0–30 days',   cls: '0', items: [] }
    ];
    ...
    var gstin = '';
    for (var k = 0; k < unpaid.length; k++) { if (unpaid[k].inv.buyerGstin) { gstin = unpaid[k].inv.buyerGstin; break; } }
    $('agingDrawerName').textContent = row.name;
    $('agingDrawerSub').textContent = gstin || ('Total outstanding: ₹' + fmtNum(row.total));

    var body = $('agingDrawerBody');
    body.textContent = '';

    buckets.forEach(function(bucket) {
      if (!bucket.items.length) return;
      var bucketTotal = bucket.items.reduce(function(s, it) { return s + it.amount; }, 0);

      var section = document.createElement('div');
      section.className = 'aging-dr-section aging-dr-section-' + bucket.cls;

      var hdr = document.createElement('div');
      hdr.className = 'aging-dr-header aging-dr-header-' + bucket.cls;
      var lbl = document.createElement('span'); lbl.textContent = bucket.label + ' — ' + bucket.items.length + ' inv';
      var tot = document.createElement('span'); tot.className = 'aging-dr-header-total'; tot.textContent = '₹' + fmtNum(bucketTotal);
    ...
    $('agingDrawerBackdrop').classList.add('cr-active');
    $('agingDrawer').classList.add('cr-active');
    document.body.style.overflow = 'hidden';
  }

  function closeAgingDrawer() {
    $('agingDrawer').classList.remove('cr-active');
    $('agingDrawerBackdrop').classList.remove('cr-active');
    document.body.style.overflow = '';
  }

  $('agingDrawerBackdrop').addEventListener('click', closeAgingDrawer);
  $('agingDrawerClose').addEventListener('click', closeAgingDrawer);
  ...
  $('crDownloadPdfBtn').addEventListener('click', async function() {
    ...
    var wrapper = document.createElement('div');
    wrapper.style.cssText = 'font-family:sans-serif;padding:24px;max-width:760px;margin:0 auto';
    container.appendChild(wrapper);
    ...
    [['#', thl + 'width:28px'], ['Customer Name', thl], ['Invoices', thr], ['Last Month (GT)', thr],
     ['Last Invoice', thr], ['Taxable', thr], ['GST', thr], ['Grand Total', thr]
    ].forEach(function(pair) {
      var th = document.createElement('th'); th.style.cssText = pair[1]; th.textContent = pair[0]; htr.appendChild(th);
    });
    ...
    visible.forEach(function(r, i) {
      ...
      vals.forEach(function(val, ci) {
        var cell = document.createElement('td');
        cell.style.cssText = ci >= 2 ? tdr : td;
        if (ci === 7) { cell.style.fontWeight = '600'; cell.style.color = '#0891b2'; }
        cell.textContent = val;
        row.appendChild(cell);
      });
      tb.appendChild(row);
    ...
    [totalCount, totalLmRev ? '₹' + fmtNum(totalLmRev) : '—', '—',
     '₹' + fmtNum(totalTax), '₹' + fmtNum(totalGst), '₹' + fmtNum(totalGrand)
  ...
  // ── Vendor Report ─────────────────────────────────────────
  var vrActivePreset  = null;
  var vrCurrentFrom   = '';
  var vrCurrentTo     = '';
  var _vrDefaultCmp   = getYearRange(-1);
  var vrCompareFrom   = _vrDefaultCmp.from;
  var vrCompareTo     = _vrDefaultCmp.to;
  var vrCompareLabel  = 'Last Year';

  var VR_COMPARE_MAP = {
    vrPresetToday:     { fn: function() { return getDayRange(-1);    }, label: 'Yesterday' },
    vrPresetYesterday: { fn: function() { return getDayRange(-2);    }, label: 'Prev Day' },
    vrPresetThisWeek:  { fn: function() { return getWeekRange(-1);   }, label: 'Last Week' },
    vrPresetLastWeek:  { fn: function() { return getWeekRange(-2);   }, label: 'Prev Week' },
    vrPresetThisMonth: { fn: function() { return getMonthRange(-1);  }, label: 'Last Month' },
    vrPresetLastMonth: { fn: function() { return getMonthRange(-2);  }, label: 'Prev Month' },
    vrPresetThisYear:  { fn: function() { return getYearRange(-1);   }, label: 'Last Year' },
    vrPresetLastYear:  { fn: function() { return getYearRange(-2);   }, label: 'Prev Year' }
  };

  function vrInvTaxable(inv) {
    var sub = 0;
    (inv.items || []).forEach(function(it) { sub += numericQtyForLine(it) * (Number(it.rate) || 0); });
    return sub;
  }
  ...
  function vrInvGrandTotal(inv) { return vrInvTaxable(inv) + vrInvGst(inv); }

  function computeVendorReportStats(from, to, cmpFrom, cmpTo) {
    var src = (from && to)
      ? poInvoices.filter(function(inv) { return inv.invoiceDate && inv.invoiceDate >= from && inv.invoiceDate <= to; })
      : poInvoices;
    ...
    var map = {};
    src.forEach(function(inv) {
      var name = (inv.vendorName || '').trim();
      if (!name) return;
      var key = name.toLowerCase();
      if (!map[key]) map[key] = { name: name, gstin: '', invoiceCount: 0, taxable: 0, gstAmount: 0, grandTotal: 0, compareGrandTotal: 0, lastDate: '', invList: [] };
      if (!map[key].gstin && inv.vendorGstin) map[key].gstin = inv.vendorGstin;
      map[key].invoiceCount += 1;
      var date = inv.invoiceDate || '';
      if (date && date > map[key].lastDate) map[key].lastDate = date;
      var taxable = vrInvTaxable(inv), gst = vrInvGst(inv);
      map[key].taxable    += taxable;
      map[key].gstAmount  += gst;
      map[key].grandTotal += taxable + gst;
      map[key].invList.push(inv);
    ...
    if (cmpFrom && cmpTo) {
      poInvoices.forEach(function(inv) {
        var key = (inv.vendorName || '').trim().toLowerCase();
        if (!map[key]) return;
        var date = inv.invoiceDate || '';
        if (date >= cmpFrom && date <= cmpTo) map[key].compareGrandTotal += vrInvGrandTotal(inv);
      });
    }
  ...
  function openVrDrawer(r) {
    ...
    $('vrDrawerBackdrop').classList.add('cr-active');
    $('vrDrawer').classList.add('cr-active');
    document.body.style.overflow = 'hidden';
  }

  function closeVrDrawer() {
    $('vrDrawer').classList.remove('cr-active');
    $('vrDrawerBackdrop').classList.remove('cr-active');
    document.body.style.overflow = '';
  }

  function renderVendorReport(from, to) {
    from = (from !== undefined) ? from : vrCurrentFrom;
    to   = (to   !== undefined) ? to   : vrCurrentTo;
    vrCurrentFrom = from;
    vrCurrentTo   = to;

    var rows    = computeVendorReportStats(from, to, vrCompareFrom, vrCompareTo);
    var query   = ($('vrSearch').value || '').toLowerCase();
    var visible = query ? rows.filter(function(r) { return r.name.toLowerCase().indexOf(query) !== -1; }) : rows;
    ...
    var tbody = $('vrTableBody');
    if (!visible.length) {
      tbody.textContent = '';
      $('vrEmpty').style.display = 'block';
      return;
    }
    $('vrEmpty').style.display = 'none';
    ...
    visible.forEach(function(r, i) {
      ...
      var tdCnt  = document.createElement('td'); tdCnt.className = 'r'; tdCnt.textContent = r.invoiceCount; tr.appendChild(tdCnt);
      var tdCmp  = document.createElement('td'); tdCmp.className  = 'r'; tdCmp.textContent  = r.compareGrandTotal ? '₹' + fmtNum(r.compareGrandTotal) : '—'; tr.appendChild(tdCmp);
      var tdDate = document.createElement('td'); tdDate.className = 'r'; tdDate.textContent = r.lastDate ? formatShortDate(r.lastDate) : '—'; tr.appendChild(tdDate);
      var tdRev  = document.createElement('td'); tdRev.className  = 'r cr-grand-cell'; tdRev.textContent = '₹' + fmtNum(r.grandTotal); tr.appendChild(tdRev);

      (function(row) { tr.addEventListener('click', function() { openVrDrawer(row); }); }(r));
      fragment.appendChild(tr);
    });
  ...
  function clearVrPreset() {
    document.querySelectorAll('#vendorReportView .apill').forEach(function(b) { b.classList.remove('active'); });
    vrActivePreset = null;
    var lyr = getYearRange(-1);
    vrCompareFrom  = lyr.from;
    vrCompareTo    = lyr.to;
  ...
  function applyVrPreset(range, btnId) {
    if (vrActivePreset === btnId) { clearVrPreset(); renderVendorReport('', ''); return; }
    clearVrPreset();
    vrActivePreset = btnId;
    $(btnId).classList.add('active');
    $('vrPresetClear').style.display = 'inline-flex';
    var cmp = VR_COMPARE_MAP[btnId];
    if (cmp) {
      var cmpRange   = cmp.fn();
      vrCompareFrom  = cmpRange.from;
      vrCompareTo    = cmpRange.to;
      vrCompareLabel = cmp.label;
    }
    renderVendorReport(range.from, range.to);
  }

  $('vrPresetToday').addEventListener('click',     function() { applyVrPreset(getDayRange(0),    'vrPresetToday'); });
  $('vrPresetYesterday').addEventListener('click', function() { applyVrPreset(getDayRange(-1),   'vrPresetYesterday'); });
  $('vrPresetThisWeek').addEventListener('click',  function() { applyVrPreset(getWeekRange(0),   'vrPresetThisWeek'); });
  $('vrPresetLastWeek').addEventListener('click',  function() { applyVrPreset(getWeekRange(-1),  'vrPresetLastWeek'); });
  $('vrPresetThisMonth').addEventListener('click', function() { applyVrPreset(getMonthRange(0),  'vrPresetThisMonth'); });
  ...
  $('vrDrawerBackdrop').addEventListener('click', closeVrDrawer);
  $('vrDrawerClose').addEventListener('click', closeVrDrawer);

  $('vrDownloadPdfBtn').addEventListener('click', async function() {
    var rows    = computeVendorReportStats(vrCurrentFrom, vrCurrentTo, vrCompareFrom, vrCompareTo);
    var query   = ($('vrSearch').value || '').toLowerCase();
    var visible = query ? rows.filter(function(r) { return r.name.toLowerCase().indexOf(query) !== -1; }) : rows;
    if (!visible.length) { alert('No data to download.'); return; }

    var presetNames = { vrPresetToday:'Today', vrPresetYesterday:'Yesterday', vrPresetThisWeek:'This week',
      vrPresetLastWeek:'Last week', vrPresetThisMonth:'This month', vrPresetLastMonth:'Last month',
      vrPresetThisYear:'This year', vrPresetLastYear:'Last year' };
    var periodLabel = (vrActivePreset && presetNames[vrActivePreset])
      ? presetNames[vrActivePreset]
      : (vrCurrentFrom && vrCurrentTo ? formatShortDate(vrCurrentFrom) + ' – ' + formatShortDate(vrCurrentTo) : 'All dates');
    ...
    var container = $('vrAnalyticsPdf'); container.textContent = '';
    var wrapper = document.createElement('div');
    wrapper.style.cssText = 'font-family:sans-serif;padding:24px;max-width:760px;margin:0 auto';
    container.appendChild(wrapper);

    var titleDiv = document.createElement('div'); titleDiv.style.marginBottom = '16px';
    var titleEl  = document.createElement('div'); titleEl.style.cssText = 'font-size:18px;font-weight:700;color:#1e293b'; titleEl.textContent = 'Vendor Report';
    var periodEl = document.createElement('div'); periodEl.style.cssText = 'font-size:13px;color:#64748b;margin-top:2px'; periodEl.textContent = 'Period: ' + periodLabel;
    titleDiv.appendChild(titleEl); titleDiv.appendChild(periodEl); wrapper.appendChild(titleDiv);

    var table = document.createElement('table'); table.style.cssText = 'width:100%;border-collapse:collapse;font-size:11px';
    var thead = document.createElement('thead'); var htr = document.createElement('tr');
    [['#', thl + 'width:28px'], ['Vendor Name', thl], ['Invoices', thr],
     [vrCompareLabel, thr], ['Last Invoice', thr], ['Taxable', thr], ['GST', thr], ['Grand Total', thr]
    ].forEach(function(pair) {
      var th = document.createElement('th'); th.style.cssText = pair[1]; th.textContent = pair[0]; htr.appendChild(th);
    });
    thead.appendChild(htr); table.appendChild(thead);
    ...
    visible.forEach(function(r, i) {
      ...
      [i + 1, r.name, r.invoiceCount,
       ...
       '₹' + fmtNum(r.taxable), '₹' + fmtNum(r.gstAmount), '₹' + fmtNum(r.grandTotal)
        ...
        var cell = document.createElement('td');
        cell.style.cssText = ci >= 2 ? tdr : td;
        if (ci === 7) { cell.style.fontWeight = '600'; cell.style.color = '#ea580c'; }
        cell.textContent = val; row.appendChild(cell);
      });
      tb.appendChild(row);
    });
    var totRow2 = document.createElement('tr'); totRow2.style.background = '#fff7ed';
    var tc1 = document.createElement('td'); tc1.colSpan = 2; tc1.style.cssText = td + 'font-weight:700;color:#ea580c'; tc1.textContent = 'Totals'; totRow2.appendChild(tc1);
    [totalCount, totalCmp ? '₹' + fmtNum(totalCmp) : '—', '—',
     '₹' + fmtNum(totalTax), '₹' + fmtNum(totalGst), '₹' + fmtNum(totalGrand)
    ].forEach(function(val, ci) {
      var c = document.createElement('td'); c.style.cssText = tdr + 'font-weight:700;color:' + (ci === 2 ? '#94a3b8' : '#ea580c');
      c.textContent = val; totRow2.appendChild(c);
    });
    tb.appendChild(totRow2); table.appendChild(tb); wrapper.appendChild(table);

    container.style.display = 'block';
    var fname = 'vendor-report-' + (vrCurrentFrom || 'all') + '.pdf';
    var opt = Object.assign({}, PDF_OPT, { margin: [0.35, 0.42, 0.35, 0.42], html2canvas: Object.assign({}, PDF_OPT.html2canvas, { scale: 1.65, scrollX: 0, scrollY: 0 }), filename: fname });
    try {
      await html2pdf().set(opt).from(container).save();
    } finally {
